  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-lg mx-auto pb-10">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 mb-6">
        <Link href="/productos" className="p-2.5 hover:bg-gray-100 rounded-2xl transition-colors">
          <ArrowLeft className="h-5 w-5 text-gray-400" />
        </Link>
        <h1 className="text-xl font-bold text-gray-900">Agregar Producto</h1>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1 space-y-3">
            <div>
              ...
              <select
                {...register('categoriaId', { required: 'Elige una' })}
                className={`w-full h-12 px-3 text-sm border-2 rounded-xl bg-white focus:border-blue-500 focus:outline-none transition-all ${errors.categoriaId ? 'border-red-400' : 'border-gray-200'}`}
              >
        ...
        <div className="bg-gray-50 rounded-2xl p-4">
          ...
          <div className="mt-3">
            ...
            <input type="number" {...register('stock', { valueAsNumber: true })}
              className="w-full h-12 px-4 text-xl font-bold text-center border-2 border-blue-200 rounded-xl bg-blue-50/30 focus:border-blue-500 focus:outline-none transition-all" placeholder="0" />
          </div>
        ...
        <button type="submit" disabled={createMutation.isPending}
          ...
          {createMutation.isPending ? (
            ...
            <><Package className="h-5 w-5" /> Guardar Producto</>
          )}
        </button>

        <Link href="/productos" className="block text-center text-sm text-gray-400 hover:text-gray-600 transition-colors py-2">
          Cancelar
        </Link>
      </motion.div>
